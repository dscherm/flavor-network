/**
 * sauceGraph.js — Build a sauce-only subgraph from Flavor Bible data
 * merged with sauce augmentation data.
 *
 * Filters the full ingredient graph to only sauce-relevant ingredients,
 * then merges in augmented nodes and edges from sauce_augment.json.
 *
 * Output format matches graph.js: { nodes: Map, edges: Array, ingredientList: string[] }
 */

import { loadSauceAugment, getSauceCategory, getSauceRole } from './sauceData.js';

/**
 * Build the sauce subgraph.
 *
 ...
 */
export async function buildSauceGraph(fullGraph, basePath = '/data') {
  const augment = await loadSauceAugment(basePath);

  // Build a set of sauce ingredient names (lowercase)
  const sauceNames = new Set(
    (augment.sauceIngredientNames || []).map(n => n.toLowerCase())
  );
  ...
  // 1. Include Flavor Bible nodes that are in the sauce list
  for (const [name, node] of fullGraph.nodes) {
    const lower = name.toLowerCase();
    if (sauceNames.has(lower) || sauceNames.has(name)) {
      const category = getSauceCategory(lower, augmentedMap);
      const codexRole = getSauceRole(lower, augmentedMap);
      nodes.set(name, {
        ...node,
        id: id++,
        sauceCategory: category,
        codexRole,
      });
    }
  }
  ...
  for (const ing of augment.ingredients || []) {
    ...
    if (!nodes.has(name)) {
      nodes.set(name, {
        ...
        embedding: null,
        position3D: null,
        sauceCategory: ing.category || 'Other',
        codexRole: ing.codexRole || 'other',
      });
    }
  }
  ...
  // 1. Include Flavor Bible edges where both endpoints are sauce ingredients
