...
- [ ] TASK-184: Focal-loss retrain to lift the still-weak odor_spicy head — tried threshold calibration (TASK-183) but F1 stuck at 0.30 with only 31 test positives. Focal loss (γ=2) may help. Low priority since 9 of 11 heads are now usable. #ml
...
- [ ] TASK-188: Consider mode/max-pool imputation for hub ingredients — current mean-pool imputation dilutes distinctive signals (mayonnaise averaged with 20 neighbors loses 'fatty' extremity). Max-pool (take top neighbor's value per task) or median-pool would preserve signal but risk overconfidence. Low priority — main surface works for ingredients with direct predictions. #ml
