...
- [ ] TASK-190: Chef simulation spec hits the per-test 5m timeout on SwiftShader baseline. Test reaches Recipe Lab tab switch (12.8s), writes its report (loadMetrics + interactions + 27 tap-target violations), then runs out of budget. Real iPhone 15 Pro will be 5-10× faster than SwiftShader so this is a SwiftShader-only artifact, but if the simulation runs in CI we either need to bump chef's per-test budget to 8m in `simulation/playwright.config.js` or trim the post-tab-switch FPS-sample window. Other 3 personas pass cleanly. #test
- [x] TASK-189b: Post-embed blend step must re-run after embed_ingredients.py. TASK-189 re-ran the GNN embedding and overwrote gnn_positions.json with raw UMAP positions (cluster separation ratio collapsed 6.07 → 0.27, making cluster colors look scrambled in Cooks With mode). Fix: `cp public/proDataset/gnn_positions.json public/proDataset/gnn_positions_raw.json && node r11-discover-depth/scripts/task3_cluster_blend_positions.cjs` — restored separation to 6.07. Future: any re-embed MUST chain the blend script. #ml #ui
- [x] TASK-189: Expanded compound coverage for hub ingredients. Ran FlavorDB scrape (935 entities, 1,788 molecules, 65 missing IDs) and extended `flavor-gnn/src/infer/embed_ingredients.py` to index FlavorDB entities as a secondary SMILES source (FooDB still wins on name collisions). Result: +790 FlavorDB entities entered the SMILES index, real-prediction coverage jumped **2,790 → 3,319 ingredients** (+529). Butter/cheddar/parmesan now produce real predictions instead of falling into the imputed bucket. `scripts/impute_hub_profiles.cjs` re-run: 562 of 594 remaining hubs imputed, only 32 skipped (orphan pairings). Shipped artifacts: `public/proDataset/gnn_entropy.json`, `gnn_entropy_imputed.json`, `gnn_positions.json`, `gnn_compounds.json`. #ml #data
