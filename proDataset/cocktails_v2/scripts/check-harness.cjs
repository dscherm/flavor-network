const r = require('../raw/corpus_v2.json');
const must = [
...
const norm = (n) => n.toLowerCase().replace(/[‘’'`]/g, '').replace(/[^\w\s-]/g, '').replace(/\s+/g, ' ').trim();
...
console.log('---');
console.log(`${hits}/${must.length} validation harness members present`);
console.log(`Total corpus: ${r.cocktails.length}`);
console.log(`IBA-official: ${r.cocktails.filter((c) => c.iba_official).length}`);
